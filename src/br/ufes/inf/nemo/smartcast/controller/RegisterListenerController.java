...
import br.ufes.inf.nemo.smartcast.application.RegisterListenerService;
...
@Named
@ConversationScoped
public class RegisterListenerController extends JSFController {
	/** Serialization id. */
	private static final long serialVersionUID = 1L;

	/** Path to the folder where the view files (web pages) for this action are placed. */
	private static final String VIEW_PATH = "/smartCast/registration/";

	/** The logger. */
	private static final Logger logger = Logger.getLogger(RegisterListenerController.class.getCanonicalName());
	...
	private Listener admin = new Listener();
...
//		logger.log(Level.FINEST, "Received input data:\n\t- admin.name = {0}\n\t- admin.email = {1}", new Object[] { admin.getName(), admin.getEmail() });
		...
		System.out.println("Entrou certo");
...
//		logger.log(Level.FINEST, "Received input data:\n\t- admin.name = {0}\n\t- admin.email = {1}", new Object[] { admin.getName(), admin.getEmail() });
	...
	//	logger.log(Level.FINEST, "Received input data:\n\t- config.institutionAcronym = {0}", config.getInstitutionAcronym());
		...
		try {
			...
			registerListenerService.installSystem(admin);
